...
extension DiagView {
    /// The static piano-roll ribbon: each held note is a horizontal bar at its pitch lane (GridSelBar.y: 0 = HIGH/top …
    /// 1 = LOW/bottom, the gridSelRollBars/drawConstellation convention), spanning its on→off time (x0…x1), thickness +
    /// opacity by velocity, in `tint` (the cell's emitter hue). Calm by construction — no animation. Used by every
    /// part / SELECT / row-selector / ferry face.
    @ViewBuilder func roomsRibbonFace(_ bars: [GridSelBar], tint: Color) -> some View {
        Canvas { ctx, size in
            ...
            for b in bars {
                ...
                let yc = b.y * (h - 4) + 2                           // pitch lane → y (GridSelBar.y=0 is HIGH → top; was wrongly flipped, notes read upside-down — Paul 2026-09-17)
