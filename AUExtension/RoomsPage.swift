...
//   • GRID + edge SELECT buttons = ONE uniform 9×9 component: every cell (grid · column-select · row-select · corner) is
//     the SAME size, filling the box. Column-select row on TOP for SELECT/PART, BOTTOM for PLAY. Row-select column on the
//     RIGHT for SELECT, LEFT for PART.
//   • The ▲PLAY door is as WIDE as the whole grid box, directly on top.
//   • SELECT / PART = 2/3 grid + 1/3 MIDI CHAIN (chain RIGHT for SELECT, LEFT for PART). The SELECT↔PART seam is on the FAR
//     side of the chain from the grid (double width).
//   • FOOTER: placeholder MIDI strips (4 IN + 4 OUT — channel · velocity light · latch). Expands to real strips → config next.
extension DiagView {
    ...
    // ── THE FOOTER — placeholder MIDI strips (4 IN + 4 OUT). Expands to real strips → MIDI config next. ──
    ...
    // ── THE UNIFORM 9×9 GRID UNIT — all cells the same size, filling the box ──
    ...
    @ViewBuilder private func launchCell9(_ r: Int, _ c: Int, selRow: Int, selCol: Int) -> some View {
        if r == selRow && c == selCol {
            Color.clear.frame(maxWidth: .infinity, maxHeight: .infinity)                          // corner
        } else if r == selRow {
            colSelCell(selCol == 0 ? c - 1 : c)                                                   // column-select (maps to the 8 grid columns)
        } else if c == selCol {
            rowSelCell()                                                                          // row-select
        } else {
            RoundedRectangle(cornerRadius: 4).fill(Color.white.opacity(0.05)).frame(maxWidth: .infinity, maxHeight: .infinity)   // grid cell
        }
    }
    ...
    // The grid column (SELECT/PART): ▲PLAY as wide as the whole box, directly on top; the uniform grid fills the rest.
    @ViewBuilder private func gridColumn(rowSelectLeft: Bool) -> some View {
        VStack(spacing: 3) {
            navPlayDoor()
            launchUnit(colSelectBottom: false, rowSelectLeft: rowSelectLeft)
        }.frame(maxWidth: .infinity, maxHeight: .infinity)
    }
    ...
    // ── THE ROOMS (side seam DOUBLED to 40) ─────────────────────────────────────────────────────────
    @ViewBuilder private func roomsSelect(_ size: CGSize) -> some View {
        GeometryReader { g in
            ...
            HStack(spacing: 6) {
                gridColumn(rowSelectLeft: false).frame(width: gridW)     // grid (2/3, left) — row-select on the RIGHT
    ...
    @ViewBuilder private func roomsPart(_ size: CGSize) -> some View {
        GeometryReader { g in
            ...
            HStack(spacing: 6) {
                ...
                gridColumn(rowSelectLeft: true).frame(width: gridW)      // grid (2/3, right) — row-select on the LEFT
            }.padding(8)
        }
    }
    @ViewBuilder private func roomsPlay(_ size: CGSize) -> some View {
        VStack(spacing: 6) {
            ...
            launchUnit(colSelectBottom: true, rowSelectLeft: false).padding(.horizontal, 10).padding(.bottom, 8)   // full-width, column-select BOTTOM
        }
    }
