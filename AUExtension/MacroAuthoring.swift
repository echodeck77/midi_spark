...
func macroParamsForProcessor(_ type: ProcessorType) -> [MacroControlParam] {
    ...
    case .octave, .transpose, .channel, .nudge, .dest, .muteMatrix, .riff, .tap, .hocket, .avoid:
        return [bypass]   // UTILITY/ROUTING/RIFF/TAP/HOCKET/AVOID (Paul 2026-08-22): edited directly (no simple macro-foldable scalar)
    }
}
