...
func macroParamsForProcessor(_ type: ProcessorType) -> [MacroControlParam] {
    ...
    case .octave, .transpose, .channel, .nudge, .dest, .muteMatrix, .riff, .tap, .hocket, .avoid, .chords, .velocity, .deal, .recorder:
        return [bypass]   // UTILITY/ROUTING/RIFF/TAP/HOCKET/AVOID/CHORDS/VELOCITY/RECORDER: edited directly (per-step lanes / discrete params — no simple macro-foldable scalar)
    }
}
