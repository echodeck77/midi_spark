...
struct DiagView: View {
    ...
    @State var buildGridSelStampSourceRow: Int? = nil    // NEW INTERFACE (Paul 2026-08-28): the ACTIVE SIDE BUTTON — the white-bordered part slot, and (if populated) the STAMP SOURCE for a long-press copy onto another side button/cell. Mutually exclusive with the other two sources.
    @State var buildRoomsCellSource: Int? = nil          // NEW INTERFACE (Paul 2026-08-28): the ACTIVE PART CELL (c*8+r) on the PART grid — the white-bordered cell + its chain as the STAMP SOURCE. Mutually exclusive with buildGridSelSel + buildGridSelStampSourceRow.
