...
- **Tab row** {#tab-bar} — the row of page tabs under the header; each opens a full-page surface (the workshop, the grid, the machine editor, the input/output setup, macros). _Why: One permanent address per surface, so a control always lives in the same place — you switch pages, you don't hunt through modes._
