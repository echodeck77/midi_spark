...
- [ ] **Fresh-cell + naming fixes (2026-07-30)** — new cells start UNROUTED (MIDI-IN + emitter A, never auto-wired to the cell above) · processor names shown in FULL. Verify a placed cell shows no source-from-above until wired; check the full name (`HARMONIZE`, 9ch) fits the desk title/picker font without clipping.
