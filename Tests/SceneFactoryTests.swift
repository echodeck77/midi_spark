...
final class SceneFactoryTests: XCTestCase {
    ...
    func testAllLoadAsV3WithMachinesAndCells() {
        for (i, s) in SceneFactory.scenes.enumerated() {
            ...
            XCTAssertEqual(doc.machines.count, 16)
    ...
    func testPacificUsesHarmonizeAndAltStates() {
        ...
        XCTAssertEqual(machine("gold").paramsB.rate, .r1_32)      // gold's designed B state
        XCTAssertTrue(machine("gold").hasProcB, "gold's B face is live (typeB set)")
        XCTAssertEqual(machine("gold").typeB, .arp)
        XCTAssertEqual(machine("vermilion").paramsB.count, 4)
        XCTAssertEqual(machine("vermilion").typeB, .ratchet, "vermilion's B face is a live ratchet")
    }
}
